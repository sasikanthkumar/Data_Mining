...
struct node{
    ...
    vector<int> node_elems; // candidate itemsets
    ...
    if (temp_node->leaf == true) {
        if (temp_node->elem_count < max_elem_count) {// no need to split
            ...
            for (set<int>::iterator it = t.begin(); it != t.end(); it++) {
                ...
                (temp_node->node_elems).push_back(*it);
            }
            ...
            set<set<int> > my_temp_set;
            my_temp_set.clear();
            for (vector<int>::iterator my_it = (temp_node->node_elems).begin(); my_it != (temp_node->node_elems).end(); my_it = my_it+max_elem_count) {
                vector<int> new_vect(my_it,my_it+max_elem_count-1);
                set<int> my_temp_s;
                for (int i = 0; i<new_vect.size(); i++) {
                    my_temp_s.insert(new_vect[i]);
                }
                my_temp_set.insert(my_temp_s);
                
            }
            
            
            for(set<set<int> >::iterator it= my_temp_set.begin();it != my_temp_set.end();it++){
                int val = 0;
                set<int> temp_set = *it;
                ...
                for (set<int>::iterator my_it = temp_set.begin(); my_it != temp_set.end(); my_it++) {
                    ((temp_node->child_ptr[val])->node_elems).push_back(*my_it);
                }
            ...
            for (set<int>::iterator my_it = t.begin(); my_it != t.end(); my_it++) {
                ((temp_node->child_ptr[hash_val])->node_elems).push_back(*my_it);
            }
...
void pass_through_hashtree(set<int> t){
    ...
    elems_in_temp_node.clear();
    
    for (vector<int>::iterator my_it = (temp_node->node_elems).begin(); my_it != (temp_node->node_elems).end(); my_it = my_it+(t.size())) {
        vector<int> new_vect(my_it,my_it+(t.size())-1);
        set<int> my_temp_s;
        for (int i = 0; i<new_vect.size(); i++) {
            my_temp_s.insert(new_vect[i]);
        }
        elems_in_temp_node.insert(my_temp_s);
