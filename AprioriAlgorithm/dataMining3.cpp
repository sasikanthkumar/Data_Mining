...
struct node * newnode(int k){  // k for no of branches and mod k same as max element count
    ...
    //cout << (temp_node->node_elems).size() << endl;
    //(temp_node->node_elems_vec).reserve(k);
    cout <<(temp_node->node_elems_vec).size() << endl;
...
void insert(set<int> t, int max_elem_count){
    if(root == NULL){
        ...
        cout << "nnn after creating node" << endl;
    ...
    if (temp_node->leaf == true) {
        if (temp_node->elem_count < max_elem_count) {// no need to split
            ...
            set<int> new_set = t;
            cout << "ddd" << endl;
            //(temp_node->node_elems_vec).push_back(&new_set);
            temp_node->node_elems_vec[0] = &new_set;
            cout << "vvv" << endl;
            ...
            for(vector<set<int> *>::iterator it= (temp_node->node_elems_vec).begin();it != (temp_node->node_elems_vec).end();it++){
                int val = 0;
                set<int> *temp_set = *it;
                ...
                ((temp_node->child_ptr[val])->node_elems_vec).push_back(&my_set);
            ...
            hash_val = (*temp_it)%max_elem_count;
            set<int> my_set1 = t;
            ((temp_node->child_ptr[hash_val])->node_elems_vec).push_back(&my_set1);
...
void pass_through_hashtree(set<int> t){
    ...
    vector<set<int> *> elems_in_temp_node_vec = temp_node->node_elems_vec;
    set<set<int> > elems_in_temp_node;
    
    for (vector<set<int> *>::iterator it_1 = elems_in_temp_node_vec.begin(); it_1 != elems_in_temp_node_vec.end(); it_1++) {
        set<int> *y = *it_1;
        elems_in_temp_node.insert(*y);
    }
